    ...
    def forward(self, inputs):
        ...
        try:
            if self.training:
                ...
                # ===== 评估模式：根据父类的不同模式分别处理 =====
                # 参考父类测试模式的逻辑，根据配置分别处理不同模式
                
                if hasattr(self.args.TRAIN, "EVAL_TEXT") and self.args.TRAIN.EVAL_TEXT:
                    # EVAL_TEXT模式：只使用文本特征，不进行视觉特征增强
                    # 这种情况下语义对齐损失计算可能不完整，但仍然可以计算
                    unique_labels = torch.unique(support_labels)
                    support_features, target_features, text_features = self.get_feats(support_images, target_images, support_real_class)
                    support_bs = support_features.shape[0]
                    target_bs = target_features.shape[0]
                    
                    # EVAL_TEXT模式下，只对target特征进行简单平均，用于语义对齐
                    target_features_enhanced = target_features  # 不使用增强，直接使用原始特征
                    support_features_enhanced = support_features  # 不使用增强，直接使用原始特征
                    
                elif hasattr(self.args.TRAIN, "COMBINE") and self.args.TRAIN.COMBINE:
                    # COMBINE模式：结合文本和视觉特征（参考父类COMBINE模式逻辑）
                    unique_labels = torch.unique(support_labels)
                    support_features, target_features, text_features = self.get_feats(support_images, target_images, support_real_class)
                    support_bs = support_features.shape[0]
                    target_bs = target_features.shape[0]
                    ...
                    # Support特征：拼接Text + Temporal Transformer增强（使用text_features_test）
                    ...
                    if hasattr(self.args.TRAIN, "MERGE_BEFORE") and self.args.TRAIN.MERGE_BEFORE:
                        ...
                        support_features_with_text = torch.cat([support_features_merged, context_support_merged], dim=1)
                        support_features_enhanced = self.context2(support_features_with_text, support_features_with_text, support_features_with_text)[:,:self.args.DATA.NUM_INPUT_FRAMES,:]
                        ...
                        support_features_with_text = torch.cat([support_features, context_support], dim=1)
                        support_features_enhanced = self.context2(support_features_with_text, support_features_with_text, support_features_with_text)[:,:self.args.DATA.NUM_INPUT_FRAMES,:]
                        
                        # 聚合同类别的support（参考父类逻辑）
                        unique_labels = torch.unique(support_labels)
