...
class CNN_OTAM_CLIPFSAR(CNN_FSHead):
    ...
    def __init__(self, cfg):
        ...
        with torch.no_grad():
            ...
            if not CNN_OTAM_CLIPFSAR._prompt_logged:
                ...
                CNN_OTAM_CLIPFSAR._prompt_logged = True

            if hasattr(self.args.TEST, "PROMPT") and self.args.TEST.PROMPT:
                text_templete = [self.args.TEST.PROMPT.format(self.class_real_train[int(ii)]) for ii in range(len(self.class_real_train))]
            else:
                text_templete = ["{}".format(self.class_real_train[int(ii)]) for ii in range(len(self.class_real_train))]
            ...
            if hasattr(self.args.TEST, "PROMPT") and self.args.TEST.PROMPT:
                ...
                text_templete = ["{}".format(self.class_real_test[int(ii)]) for ii in range(len(self.class_real_test))]
            text_templete = tokenize(text_templete).cuda()
        ...
        if hasattr(self.args.TRAIN, "TEXT_COFF") and self.args.TRAIN.TEXT_COFF is not None:
            ...
            logger.info(f"CNN_OTAM_CLIPFSAR initialized with TEXT_COFF parameter: β={beta}, (1-β)={1-beta}")
        else:
            logger.info("CNN_OTAM_CLIPFSAR initialized with default fusion weights (no TEXT_COFF parameter)")
        # set_trace()
    ...
    def forward(self, inputs):
        ...
        if self.training:
            ...
            if hasattr(self.args.TRAIN, "MERGE_BEFORE") and self.args.TRAIN.MERGE_BEFORE:
                unique_labels = torch.unique(support_labels)
                support_features = [torch.mean(torch.index_select(support_features, 0, extract_class_indices(support_labels, c)), dim=0) for c in unique_labels]
                #labels = torch.tensor([3, 1, 3, 2, 1])
                # idx = extract_class_indices(labels, 1)
                # print(idx)  # tensor([1, 4])
                support_features = torch.stack(support_features)
