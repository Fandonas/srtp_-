...
class CNN_SEMANTIC_ALIGNMENT_FEW_SHOT_IMPROVED(CNN_OTAM_CLIPFSAR):
    """
    改进版小样本学习的语义对齐模型
    ...
    5. 正确的视频文本编码：使用"a video of"而非"a photo of"
    6. 增强的评估模式支持
    ...
    def __init__(self, cfg):
        super(CNN_SEMANTIC_ALIGNMENT_FEW_SHOT_IMPROVED, self).__init__(cfg)
        ...
        # 评估模式下是否计算语义对齐损失（用于分析）
        self.eval_semantic_loss = getattr(cfg.TRAIN, 'EVAL_SEMANTIC_LOSS', False)
    ...
    def forward(self, inputs):
        ...
        # 获取视频特征用于语义对齐
        try:
            # 【改进】训练模式或配置了评估语义损失时才计算
            if self.training or self.eval_semantic_loss:
    ...
    def loss(self, task_dict, model_dict):
        ...
        return {
            ...
            'weighted_semantic_loss': self.semantic_loss_weight * semantic_loss if self.training else torch.tensor(0.0, device=base_loss.device)
        }