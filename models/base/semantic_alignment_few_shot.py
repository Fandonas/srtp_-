    ...
    def forward(self, inputs):
        ...
        if self.training:
            ...
            if self.use_query_embed_train:
                ...
                target_context = self.query_embed_train.weight[target_real_class.long()].unsqueeze(1)
            else:
                context_support = self.text_features_train[support_real_class.long()].unsqueeze(1)
                target_context = self.text_features_train[target_real_class.long()].unsqueeze(1)
            ...
            if self.use_mid_layer_train:
                ...
                target_context1 = self.mid_layer(target_context)
            else:
                context_support1 = context_support
                target_context1 = target_context
            ...
            target_features_ctx = self.context2(
                torch.cat([target_features_raw, target_context1], dim=1),
                torch.cat([target_features_raw, target_context1], dim=1),
                torch.cat([target_features_raw, target_context1], dim=1)
            ...
            if use_eval_text or use_combine:
                ...
                if self.use_query_embed_eval:
                    ...
                    target_context = self.text_features_test[target_real_class.long()].unsqueeze(1)
