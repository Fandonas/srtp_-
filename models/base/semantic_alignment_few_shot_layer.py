...
class CNN_SEMANTIC_ALIGNMENT_FEW_SHOT(CNN_OTAM_CLIPFSAR):
    ...
    def __init__(self, cfg):
        ...
        elif self.use_query_embed_eval:
            # 仅评估时使用：冻结权重，DDP 不追踪此参数
            self.query_embed_train = nn.Embedding(num_train_classes, self.mid_dim)
            with torch.no_grad():
                self.query_embed_train.weight.copy_(self.text_features_train)
            self.query_embed_train.weight.requires_grad_(False)
        # 两者均为 False：不创建此模块，彻底规避 DDP 未使用参数报错
            ...
            if hasattr(self.args.TRAIN, "MERGE_BEFORE") and self.args.TRAIN.MERGE_BEFORE:
                ...
                if self.use_mid_layer2_semantic_train and self.use_mid_layer2:
                    ctx_merged2 = torch.stack([
                        torch.mean(torch.index_select(context_support2, 0, extract_class_indices(support_labels, c)), dim=0)
                        for c in unique_labels])
